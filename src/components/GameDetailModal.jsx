...
function GameDetailModal({ game, allTags, onClose, onUpdate }) {
  ...
  const handleDeleteGame = async (deleteFiles = false) => {
    try {
      const ok = await window.electronAPI.deleteGame(game.id, deleteFiles);
      if (!ok) {
        ...
        <div className="relative z-10 flex flex-col min-h-[96vh] max-h-[98vh]">
          ...
          <div>
            ...
            <input
              ...
              placeholder=""
              className="w-full bg-black/5 border border-white/5 rounded-lg px-3 py-2.5 text-white text-sm focus:outline-none focus:ring-2 focus:ring-white/20 focus:border-transparent transition-all duration-300"
            />
          </div>
                      ...
                      >
                        <div className="flex items-center justify-between gap-3">
                          <div className="min-w-0">
                            ...
                            <div className="text-xs text-gray-500 break-all font-mono">{item.path || item.relativePath}</div>
                          </div>
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-2xl w-full max-w-sm p-6 shadow-2xl transform animate-in zoom-in-95 duration-200">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 bg-gray-800 hover:bg-gray-700 text-gray-300 font-medium py-2.5 rounded-xl transition-all text-xs sm:text-sm"
              >
                取消
              </button>
              <button
                onClick={() => handleDeleteGame(false)}
                className="flex-[1.2] bg-gradient-to-r from-red-600 to-pink-600 text-white font-medium py-2.5 rounded-xl hover:shadow-lg hover:shadow-red-600/30 transition-all transform active:scale-95 text-xs sm:text-sm whitespace-nowrap"
              >
                确定删除
              </button>
              {/* <button
